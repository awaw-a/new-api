...
export function Features(_props: FeaturesProps) {
  ...
  const features = [
        ...
        <div className='mt-4 grid grid-cols-3 gap-2'>
          {['OpenAI', 'Claude', 'Gemini', 'DeepSeek', 'Qwen', 'Llama'].map(
            (name) => (
              <div
                key={name}
                className='border-border/30 bg-muted/20 text-muted-foreground flex items-center justify-center rounded-lg border px-3 py-2 text-xs transition-colors duration-300 hover:border-emerald-500/30 hover:bg-emerald-500/5'
              >
                {name}
              </div>
            )
    ...
    },
    {
      ...
      span: 'md:col-span-1',
      icon: <Globe className='size-4 text-lime-400' />,
      visual: (
